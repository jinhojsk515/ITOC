...
def main():
    ...
    sd3 = StableDiffusion3Base(NFE=28, device=device, dtype=torch.float16, deterministic=DETERMINISTIC)
    ...
    sd3.vae.encoder.to('cpu')
    del sd3.text_enc_1, sd3.text_enc_2, sd3.text_enc_3, sd3.vae.encoder
    ...
    if method == "gradient_ascent":
        ...
        for i in pbar:
            reward_grads, reward_values = all_grad_rewards(sd3, image, prompts, REWARD_NAME, METHOD_NAME)
    ...
    # ==================== DPS, FreeDoM, TFG ==================== #
        ...
        with torch.no_grad():
            for i, t in enumerate(pbar):
                ...
                def rewardfn(x0):
                    reward_grads, reward_values = all_grad_rewards(sd3, x0, prompts, REWARD_NAME, METHOD_NAME)
    ...
    # ==================== Ours ==================== #
        ...
        us_32 = torch.zeros_like(z_source_traj[:, 1:]).to(dtype=torch.float32)
        ...
        b_grad_all = None       # we cache the base drift term's gradient w.r.t. x_t to use in adjoint computation
        with torch.no_grad():
            for i in pbar:
                rg, rv = all_grad_rewards(sd3, z_source_traj[:, -1], prompts, REWARD_NAME, METHOD_NAME)
                reward_all.append(rv.item())

                # ===== compute adjoint state p_t ===== #
                adjoint_states, reward_values, _, all_prev_pred_init = sd3.compute_adjoints(z_source_traj, all_timesteps.unsqueeze(-1),
                ...
                if DETERMINISTIC:   u_target = -adjoint_states[:, :-1] * all_dts
                else:   u_target = -adjoint_states[:, :-1] * all_dts
                with torch.enable_grad():
                    optimizer.zero_grad()
                    loss = ((us_32 - u_target.detach()) ** 2).mean()
                    (loss*1.).backward()
                    optimizer.step()
                    scheduler.step()
                ...
                for k in range(all_timesteps.shape[0]):
                    ...
                    next = (prev_sample_init + noises[:, k] + us_32[:, k]).detach().to(sd3.dtype)
